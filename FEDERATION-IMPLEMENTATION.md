...
This document is the **phased implementation plan** for the relay federation, bootstrap resilience, and anti-censorship features specified in **PNP-008** (`specs/PNP-008-relay-federation.md`). It tracks *where code changes land*; the spec tracks *what must be true on the wire*.

> **Status (2026-04-17):** Current code includes authority keys, endorsed descriptors, `/endorse`, `/directory`, and simple polling-based relay directory sync through configured `PEER_RELAY_URLS`. It does not yet implement the full federation manager, bridge relay design, health/reputation system, or pluggable transports described below.

## Motivation

ParolNet relay discovery is gossip-based (no centralized authority) — good foundation. Current design gaps for censorship resistance:
...
- Wire format normative in PNP-008 §4
...
- Sizing rules normative in PNP-008 §6.2
...
- Reputation update rules normative in PNP-008 §7
...
Fallback chain and channel wire formats normative in PNP-008 §8.
...
State machine and timers normative in PNP-008 §5.

**Conflict resolution**: Newer timestamp wins. Same-timestamp tie-break: lexicographic PeerId. (PNP-008 §6.4)

**Rate limiting**: Per-peer token bucket — 100 descriptors/min, 10 sync requests/hr. Exceeding penalizes via existing `PeerScore`.

**Eclipse attack mitigation**: Min 3 federation peers from distinct /16 subnets (extends existing subnet diversity logic). (PNP-008 §10.2)
...
- `BridgeDescriptor`: like `RelayDescriptor` but NOT published in main directory (PNP-008 §9)
